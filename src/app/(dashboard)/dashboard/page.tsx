...
import {
    getClients, getProjects, getUsers, getNotices,
    addKudos, onKudos,
    Project, Notice, Kudos
} from "@/lib/db";

import { Users, Briefcase, Activity as ActivityIcon, CheckCircle2, Megaphone, Send, Sparkles, Heart } from "lucide-react";
...
export default function DashboardPage() {
    ...
    // Stats
    const [stats, setStats] = useState({ totalClients: 0, totalProjects: 0, activeProjects: 0, totalUsers: 0 });
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            const active = notices.filter(n => (Date.now() - n.createdAt) <= 48 * 60 * 60 * 1000).sort((a, b) => b.createdAt - a.createdAt);
    ...
    const handleSendKudos = async () => {
        ...
        const themeColors = ['bg-yellow-200 text-yellow-900', 'bg-blue-200 text-blue-900', 'bg-pink-200 text-pink-900', 'bg-emerald-200 text-emerald-900', 'bg-purple-200 text-purple-900'];
    ...
    const statCards = [
        { name: "Total Projects", value: stats.totalProjects, icon: Briefcase, color: "text-blue-600 dark:text-blue-400", bg: "bg-blue-100 dark:bg-blue-900/40" },
        { name: "Active Projects", value: stats.activeProjects, icon: ActivityIcon, color: "text-orange-600 dark:text-orange-400", bg: "bg-orange-100 dark:bg-orange-900/40" },
        { name: "Total Clients", value: stats.totalClients, icon: Users, color: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-100 dark:bg-emerald-900/40" },
        { name: "Team Members", value: stats.totalUsers, icon: CheckCircle2, color: "text-purple-600 dark:text-purple-400", bg: "bg-purple-100 dark:bg-purple-900/40" },
    ];
    ...
    return (
        <div className="space-y-8 relative">
            ...
            <div>
                ...
                <p className="mt-2 text-gray-500 dark:text-gray-400 text-lg">Welcome back to your workspace</p>
            </div>
                ...
                >
                    ...
                    <div className="flex items-start gap-4">
                        <div className="bg-white dark:bg-gray-800 p-3 rounded-xl shadow-sm shrink-0">
                            <Megaphone className={`w-6 h-6 text-blue-600 dark:text-blue-400 ${!isNoticePaused && 'animate-pulse'}`} />
                        </div>
                        <div className="flex-1 min-w-0">
                            <div className="flex justify-between items-start mb-2 gap-2">
                                ...
                                <div className="flex items-center gap-2 shrink-0">
                                    {isNoticePaused && activeNotices.length > 1 && (
                                        <span className="text-[10px] uppercase font-bold text-blue-500 dark:text-blue-400">Paused</span>
                                    )}
                            ...
                            <p className="text-gray-700 dark:text-gray-300 text-sm leading-relaxed whitespace-pre-wrap">{activeNotices[currentNoticeIndex].message}</p>
                            <p className="mt-3 text-xs font-medium text-gray-500 dark:text-gray-400">Published by {activeNotices[currentNoticeIndex].createdBy}</p>
                        </div>
                    </div>
                    ...
                    {activeNotices.length > 1 && (
                        <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1.5 opacity-60 group-hover:opacity-100 transition-opacity">
                            {activeNotices.map((_, i) => (
                                <div key={i} className={`w-1.5 h-1.5 rounded-full transition-colors ${i === currentNoticeIndex ? 'bg-blue-500' : 'bg-blue-200 dark:bg-blue-800'}`} />
                            ))}
                        </div>
                    )}
                </div>
                ...
                {statCards.map((stat) => (
                    ...
                    >
                        ...
                        <div className="min-w-0">
                            <p className="text-xs sm:text-sm font-medium text-gray-500 dark:text-gray-400 leading-tight">{stat.name}</p>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-5 gap-6 xl:gap-8">
                    ...
                    {recentProjects.length === 0 ? (
                        ...
                        <div className="flex flex-col gap-3">
                            {recentProjects.map((project) => (
                                <div key={project.id} className="flex items-center justify-between p-4 rounded-xl border border-gray-100 dark:border-gray-800/60 bg-gray-50/50 dark:bg-gray-800/20 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                    <div className="flex flex-col min-w-0 mr-4">
                                        <h3 className="text-sm font-bold text-gray-900 dark:text-white truncate">{project.projectName}</h3>
                                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{project.serviceType}</p>
                                    </div>
                                    <div className="shrink-0">
                                        <span className={`inline-flex items-center px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider border ${project.status === "Completed"
                ...
                <div className="xl:col-span-2 bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-900/10 dark:to-purple-900/10 rounded-2xl shadow-sm border border-indigo-100 dark:border-indigo-800/50 flex flex-col relative overflow-hidden" style={{ minHeight: "360px", maxHeight: "480px" }}>
                    ...
                    <div className="flex items-center gap-3 px-5 py-4 border-b border-indigo-100/50 dark:border-indigo-800/30 relative z-10">
                        ...
                        <div>
                            <h2 className="text-sm font-bold text-gray-900 dark:text-white">Kudos Board</h2>
                            <p className="text-xs text-gray-600 dark:text-gray-400 mt-0.5">Celebrate the team's wins!</p>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-5 relative z-10 custom-scrollbar">
                        {kudosList.length === 0 ? (
                            <div className="flex flex-col items-center justify-center h-full text-center py-8 text-indigo-400/50 dark:text-indigo-500/30">
                                ...
                                <p className="text-xs mt-1 text-gray-400 dark:text-gray-500">Be the first to shoutout a teammate!</p>
                            </div>
                        ) : (
                                    ...
                                    return (
                                        ...
                                        >
                                            <p className="font-handwriting text-[15px] leading-relaxed mb-3 font-medium opacity-90">
                                                ...
                                                {kudos.text.split(/(@[a-zA-Z0-9_\-]+)/g).map((part, index) => {
                                                    if (part.startsWith('@')) {
                                                        return <span key={index} className="font-bold opacity-100 bg-white/30 px-1 rounded-md">{part}</span>;
                                                    }
                                                    return <span key={index}>{part}</span>;
                                                })}
                                            </p>
                                            <div className="flex items-center justify-between mt-auto">
                                                <span className="text-xs font-bold opacity-75">— {kudos.fromUser.split(" ")[0]}</span>
                                                <span className="text-[9px] font-bold opacity-40 uppercase tracking-widest">{formatTime(kudos.createdAt)}</span>
                                            </div>
                                        </div>
                                    )
                                })}
